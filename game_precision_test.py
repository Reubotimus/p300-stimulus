...
n_cycles_in_epoch = 1 # number of times each row and column will be cycled through before a break
...
displayed_chars = "abcdefghijklmnopqrstuvwxyz0123456789".upper()
...
while True:
    ...
    if epoch_on:
        ...
        if time_of_frame - time_since_intensification > stimulus_interval:
            ...
            if group_num == matrix_dimensions[0]+matrix_dimensions[1]:
                ...
                shuffled = False
                last_row = groups[group_num - 1]
                while not shuffled:
                    random.shuffle(groups)
                    if last_row != groups[0]:
                        shuffled = True
                group_num = 0
                n_cycles += 1
            # intensifies new group
            for char_index in groups[group_num]:
                chars[char_index].intensify()
            # Intensifies test square
            test_square.intensify()
            # refreshes and puts all characters on screen
            screen.fill('black')
            screen.blit(test_square.surface, test_square.screen_position)
            for char in chars:
    ...
    if row_intensified and time_of_frame - time_since_intensification > intensification_duration:
        # darkens group
        for char_index in groups[group_num - 1]:
            chars[char_index].darken()
        # Intensifies test square
        test_square.darken()
        # refreshes and puts all characters on screen
        screen.fill('black')
